### This script is borrowed from CLAM (https://github.com/mahmoodlab/CLAM)
import math
import os
import time
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def saveSegmentation(self, mask_file):
        if mask_file.endswith('.pkl'):
        ...
        elif mask_file.endswith('.geojson'):
            import json
            # Flip x and y since this contour is on cv2 coordinate system
            coordinates = [coord.squeeze().tolist() for coord in self.contours_tissue]
            for i in range(len(self.holes_tissue)):
                for j in range(len(self.holes_tissue[i])):
                    coordinates.append(self.holes_tissue[i][j].squeeze().tolist())
            ...
            NotImplementedError("Only .pkl and .geojson file formats are supported for saving segmentation results")
