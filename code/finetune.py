...
parser.add_argument('--finetune_gene_list', type=str, default=None, help='Path to the gene list for finetuning')
...
dataset_dir = f"{main_dir}/{args.train_path}/processed_pkl_finetune"
...
gene_list = pd.read_csv(f"{main_dir}/{args.finetune_gene_list}",header=None).iloc[:,0].values.tolist()
...
num_epochs = exp_epochs + args.num_epochs #parameters.get('num_epochs', 600)
...
model = AURORA(input_size, output_size, bulk_size=len(hvg_list),
                ...
                use_bulk=use_bulk, pred_celltype=pred_celltype, finetune=True)
model.to(device)
# Training loop
...
old_state_dict = torch.load(f"{main_dir}/{args.model_pth}", weights_only=False, map_location=device)
...
for para in model.new_decoder.parameters():
...
for epoch in range(start_epoch,num_epochs):
    ...
    if epoch == exp_epochs:
        ...
        optimizer = optim.Adam(model.new_decoder.parameters(), lr=lr_prop, weight_decay=weight_decay)
    ...
    writer.add_scalar('training global bulk loss',
                    ...
                    epoch)
    if epoch % 100 == 0:
        filename = f"{main_dir}/{args.train_path}/checkpoints/AURORA_finetune_{epoch}.pth"
...
torch.save(state_dict, f"{main_dir}/{args.train_path}/AURORA_finetune_model_weights.pth")
