...
def get_data(prefix, use_tif=False):
    ...
    locs = get_locs(prefix, target_shape=embs.shape[:2], use_tif=use_tif)
...
def predict(
        model_states, x_batches, name_list, y_range, prefix,suffix,
        device='cuda'):
    ...
    save_pickle(
            z_dict,
            prefix+f'embeddings-gene{suffix}.pickle')
    del z_point
    ...
    for idx_grp in idx_groups:
        ...
        for i, name in enumerate(name_grp):
            save_pickle(y_grp[..., i], f'{prefix}cnts-super{suffix}/{name}.pickle')


def impute(
        embs, cnts, locs, radius, epochs, batch_size, prefix, suffix,
    ...
    predict(
            ...
            prefix=prefix, suffix=suffix, device=device)
...
def get_args():
    ...
    parser.add_argument('--tif', action='store_true')
...
def main():
    args = get_args()
    embs, cnts, locs = get_data(args.prefix, use_tif=args.tif)
    args = get_args()
